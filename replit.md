...
**AI Legal Guidance Engine**: The platform features a dual-mode guidance system that intelligently selects between Claude AI (Anthropic) and a rule-based engine. When users provide detailed incident descriptions or specific concerns, Claude Sonnet 4.5 generates personalized, context-aware legal guidance using advanced natural language processing. The system includes automatic retry logic that retries once on timeout, significantly improving reliability. It automatically falls back to the rule-based engine if AI is unavailable after retries or if users provide minimal details, ensuring consistent service. All AI interactions include usage tracking (input/output tokens, cost estimation) logged server-side for transparency and cost monitoring. Uses Replit AI Integrations for Anthropic access (no personal API key required, charges to Replit credits).
